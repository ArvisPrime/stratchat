...
import { Mic, MicOff, Download, Zap, Brain, LayoutDashboard, MessageSquareText, Headphones, VolumeX, Menu, Settings, Moon, Sun } from 'lucide-react';
import { GeminiLiveService } from './services/geminiLive';
import { getQuickSummary, getDeepAnalysis, generateStrategicQuestion, refineTranscript } from './services/geminiStatic';
import { TranscriptView } from './components/TranscriptView';
import { CoachView } from './components/CoachView';
import { AnalysisPanel } from './components/AnalysisPanel';
import { SettingsDialog } from './components/SettingsDialog';
import { TranscriptEntry, ConnectionStatus, CoachSuggestion, AnalysisResult } from './types';
...
export default function App() {
  ...
  const handleNewSuggestion = useCallback((text: string, type: 'auto' | 'manual') => {
    ...
    liveServiceRef.current = new GeminiLiveService({
      ...
      onTranscript: (entry) => {
        if (entry.speaker === 'ai') {
          ...
          handleNewSuggestion(entry.text, 'auto');
        } else {
          setTranscript(prev => {
  ...
  const handleGenerateQuestion = async () => {
    ...
    try {
      ...
      handleNewSuggestion(question, 'manual');
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="flex flex-col h-[100dvh] bg-background text-foreground font-sans overflow-hidden relative transition-colors duration-300">
      ...
      <div className="absolute bottom-20 md:bottom-8 right-6 z-40">
        <button
          ...
          className={`h-14 w-14 md:h-16 md:w-16 rounded-full flex items-center justify-center shadow-xl shadow-primary/20 transition-all duration-300 ${
            isConnected
