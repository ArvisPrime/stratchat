...
export const TranscriptView: React.FC<TranscriptViewProps> = ({ entries }) => {
  ...
  return (
    <div className="h-full flex flex-col p-4 pb-24 md:pb-4">
      <Virtuoso
        ref={virtuosoRef}
        data={chatEntries}
        followOutput={'auto'}
        initialTopMostItemIndex={chatEntries.length - 1}
        itemContent={(index, entry) => {
          ...
          return (
            <div className={`flex flex-col mb-6 ${isUser ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
              ...
              {/* Bubble */}
              <div className={`relative max-w-[90%] md:max-w-[80%] rounded-2xl px-5 py-3.5 text-sm leading-relaxed shadow-sm transition-colors ${isStreaming
